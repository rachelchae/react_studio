...
function App() {
  ...
  const [productGallery, setProductGallery] = useState([...bakeryData]);
          ...
          <Col md="8"> 
            <div className="gallery"> 
              {sortedArray.filter(matchesFilterType).map((item) => ( 
                <ProductItem item={item} addToCart={addToCart} removeFromCart={removeFromCart} cart={cartItems} /> // this maps each bakery item to its data!
              ))}
            </div>
          </Col>
          {/*CART*/}
